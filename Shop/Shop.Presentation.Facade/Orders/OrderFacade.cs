...
public class OrderFacade : IOrderFacade
{
    ...
    public async Task<OperationResult> OrderCheckout(ChechOutOrderCommand command)
    {
        return await _mediator.Send(command);
    }

    public async Task<OperationResult> RemoveOrderItem(RemoveOrdertemCommand command)
    {
        return await _mediator.Send(command);
    }
}